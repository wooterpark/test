...
)
{
  ...
  if(RetVal == (Std_ReturnType)E_OK)
  {
    ...
    /*Frank Edit for enable intelnal watchdog timer when OCDS is enabled*/
    //if(!(MODULE_CBS.OSTATE&0x1UL))
    {
		MODULE_CBS.OEC =0xA1;
		MODULE_CBS.OEC =0x5E;
		MODULE_CBS.OEC =0xA1;
		...
		MODULE_CBS.OCNTRL |= 0x3000UL;
    }
#if 0
    /* application reset for SW reset, application reset for SMU, disable ESR0 reset */
    SCU_RSTCON.U=0x0280;
    SCU_ARSTDIS.U = 0x7uL;
    /* in case of PowerOnReset the following bits are set: PORST, EVR33, EVR13, SWD (supply watchdog) */
    /* The bits must be cleared manually                                                              */
    SCU_RSTCON2.B.CLRC=1;

    /* ESRCFG0, ESRCFG1: filter is bypassed, no trigger is generated */
    SCU_ESRCFG0.U = 0uL;
    SCU_ESRCFG1.U = 0uL;

    /* ESR output configuration */
    SCU_ESROCFG.U = 0x00000000uL;
    /* IOCR input/output control                    */
	/* ESR0: open-drain, ESR1: input pull-up device connected */
    SCU_IOCR.U = 0x20E0uL;
#endif
