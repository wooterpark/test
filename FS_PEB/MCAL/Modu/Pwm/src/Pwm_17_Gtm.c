...
#endif /* (end of PWM_PB_FIXEDADDR == STD_ON) */
...
)
{
  ...
  /* Mask to clear channel specific bits in GLB register */
  MaskValue =   ((uint32)~((uint32)PWM_CONST_3 << (uint16)(PWM_CONST_16 + (\
        PWM_CONST_2 * ((uint16)ChannelNumber % GTM_TOM_CHANNELS_PER_TGC)))));
  /* Clear the reserve bits in the mask value */
  MaskValue =  (uint32) PWM_TGC_GLB_CLR_MASK & MaskValue;
            ...
            (PwmProperty.ShiftedSpl == PWM_SHIFTED_RELATED_CHANNEL))
  {
    ...
    if(PwmProperty.Coherency == PWM_NON_COHERENT)
    {
      ...
      Pwm_lSchmExitStartChannel();
    }
    else
    {
      /* Disable the update from shadow register */
      RegVal = ((uint32)PWM_ONE_U << (uint16)(PWM_CONST_16 + (\
        PWM_CONST_2 * ((uint16)ChannelNumber % GTM_TOM_CHANNELS_PER_TGC))));

      PWM_SFR_RUNTIME_USER_MODE_MODIFY32((TomTgcRegPtr->GLB_CTRL.U),\
            MaskValue,RegVal)
      ...
      /* Enable the update from shadow register */
      RegVal = ((uint32)PWM_CONST_2 << (uint16)(PWM_CONST_16 + (\
        PWM_CONST_2 * ((uint16)ChannelNumber % GTM_TOM_CHANNELS_PER_TGC))));
      PWM_SFR_RUNTIME_USER_MODE_MODIFY32((TomTgcRegPtr->GLB_CTRL.U),\
            MaskValue,RegVal)
...
)
{
            ...
            (PwmProperty.ShiftedSpl == PWM_SHIFTED_RELATED_CHANNEL))
  {
    ...
    else
    {
      ...
      /*Disable update from shadow registers */
      RegVal = ((uint32)PWM_ONE_U << (uint16)(PWM_CONST_16 + (\
        PWM_CONST_2 * ((uint16)ChannelNumber % GTM_ATOM_CHANNELS_PER_AGC))));
      PWM_SFR_RUNTIME_USER_MODE_MODIFY32((AtomRegPtr->AGC.GLB_CTRL.U),\
        MaskValue,RegVal)
      ...
      /*Enable update from shadow registers */
      RegVal = ((uint32)PWM_CONST_2 << (uint16)(PWM_CONST_16 + (\
        PWM_CONST_2 * ((uint16)ChannelNumber % GTM_ATOM_CHANNELS_PER_AGC))));
      PWM_SFR_RUNTIME_USER_MODE_MODIFY32((AtomRegPtr->AGC.GLB_CTRL.U),\
        MaskValue,RegVal)
...
/*******************************************************************************
**                                                                            **
** Syntax : static Pwm_ChannelIdentifierType Pwm_lGtmChannelIdentifier        **
**                           (Pwm_GlobalChannelNumberType GlobalChannelNumber)**
**                                                                            **
** Service ID:    None                                                        **
**                                                                            **
                                    ...
                                    uint16 CohAndMax)
{
  ...
  for(LoopVar=0U;LoopVar<MaxGroupChannels;LoopVar++)
  {
    ...
    GroupNumber = (uint32) ChannelNumber / GTM_TOM_CHANNELS_PER_TGC;
    TomTgcRegPtr = &((*(Ifx_GTM_TOMx*)(volatile void*)\
                    (MODULE_GTM.TOM)).TOM_TGC[ModuleNumber].TGC[GroupNumber]);
    TomChannelRegPtr = &(((*(Ifx_GTM_TOMx*)(volatile void*) \
                   (MODULE_GTM.TOM)).TOM_CH[ModuleNumber].CH[ChannelNumber]));
...
**                                                                           **
** Parameters (in):  ModChannelNo : PWM Channel number which caused the      **
**                                          interrpt                         **
**                                                                           **
** Parameters (out): none                                                    **
**                                                                           **
...
**                                                                           **
******************************************************************************/
void Pwm_17_Gtm_Isr(uint8 ModChannelNo)
{
  #if (PWM_NOTIFICATION_SUPPORTED == STD_ON)
  ...
  uint8 ErrorFlag;
  #endif
  Pwm_17_Gtm_ChannelType PwmChannelnumber = ModChannelNo;
  #if (PWM_SAFETY_ENABLE == STD_ON)
  ErrorFlag = 0U;
