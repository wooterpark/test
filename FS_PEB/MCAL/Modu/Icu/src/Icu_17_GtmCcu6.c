...
/*  TRACEABILITY: [cover parentID= DS_AS_ICU213_1,DS_NAS_ICU_PR228,
  ...
  DS_AS_ICU085,SAS_MCAL_ICU_0034,SAS_MCAL_ICU_0028,DS_MCAL_ICU_9012,
  DS_MCAL_ICU_9013,DS_MCAL_ICU_9007_1,DS_MCAL_ICU_9007_2,DS_MCAL_ICU_9007_3]
                   [/cover]                                                  */
...
/* ((ICU_SET_MODE_API == STD_ON) && ((ICU_TIM_SIGEDGE_USED == STD_ON) || \
     (ICU_ERU_USED == STD_ON) || (ICU_CCU6_SIGEDGE_USED == STD_ON))) */
...
static void Icu_lTimeStampBufferLimitCheck(
 ...
 /* This function has been introduced in order to 
    resolve the RSM warning (#55: exceeding the limit 
    for the depth of scope)*/
static void Icu_lTimeStampHandler(
                uint8 ChannelType, uint8 Channel);
#endif
...
#endif
/* ((ICU_ERU_USED == STD_ON) || (ICU_CCU_USED == STD_ON)) */
...
/* ((ICU_GET_INPUT_STATE_API == STD_ON) && (ICU_TIM_SIGMEAS_USED == STD_ON)\
&& (ICU_SIGNAL_MEASUREMENT_API == STD_ON)) */
...
/* ((ICU_SET_MODE_API == STD_ON) && (ICU_CCU6_SIGEDGE_USED == STD_ON)) */
...
void Icu_17_GtmCcu6_DeInit(void)
{
  ...
  #endif
  {
    ...
    #if ((ICU_ERU_USED == STD_ON) || (ICU_CCU6_USED == STD_ON))
    /* Initialize ICU configured channel pointer */
...
/* (ICU_CCU6_USED == STD_ON) */
    #endif
/* ((ICU_ERU_USED == STD_ON) || (ICU_CCU6_USED == STD_ON)) */
...
/* (ICU_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    if (Mode == ICU_MODE_SLEEP)
    {
      ...
      /* (ICU_EDGE_DETECTION_USED == STD_ON) */
...
)
{
  ...
  uint8 IcuOption;
  #endif
  /* (ICU_SET_ACTIVATION_REQD == STD_ON) */
...
/*  (ICU_DEV_ERROR_DETECT == STD_ON) */
 ...
 PBConfigStructure and is within allowed range.*/
    ...
    if((uint32)(LocalIcuChanPtr->IcuProperties.AssignedHwUnit)== ICU_GTM_OPTION)
    {
      /* Clearing of pending interrupts and re-enabling of interrupts are 
         done only if the MeasurementMode is TimeStamp mode. This condition 
         is introduced as per Jira 0000051018-519 */
      if (LocalIcuChanPtr->IcuProperties.MeasurementMode != ICU_MODE_TIMESTAMP)
      {
        /* Initialize ICU configured channel pointer */
        HwUnit = (uint8)(LocalIcuChanPtr->IcuProperties.AssignedHwUnitNumber);
                          ...
                          )
{
  ...
  uint8 ChlType;
  /* Pointer to Channel Configuration structure */
  const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanBufPtr; 
#endif 
...
#endif /* (ICU_ASIL_TIM_TIMESTAMP_USED == STD_ON) */
#endif /* (ICU_SAFETY_ENABLE == STD_ON) */
   {
...
/* #if(ICU_TIM_SIGMEAS_USED == STD_ON) */
 ...
 PBConfigStructure and is within allowed range.*/
    ...
    ChlType = Icu_lGetChannelType(Channel);
    /* ICU050: Initialize ICU configured channel pointer */
    /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to 
 PBConfigStructure and is within allowed range.*/
    LocalIcuChanPtr = (Icu_kConfigPtr->ChannelConfigPtr) + Channel ;
    /* Extract the actual cell number */
...
#endif
/* ((ICU_EDGE_DETECT_API == STD_ON) && (ICU_DEV_ERROR_DETECT == STD_ON)) */
...
static void Icu_lSetNormal( void )
{
  ...
  Icu_17_GtmCcu6_ActivationType Edge;
  #endif
  /* ((ICU_TIM_SIGEDGE_USED == STD_ON) || (ICU_ERU_USED == STD_ON)) */
...
/* ((ICU_EDGE_DETECT_API == STD_ON) && (ICU_GTM_TIM_USED == STD_ON)) */
...
/* (((ICU_SET_MODE_API == STD_ON) || (ICU_EDGE_DETECT_API == STD_ON)) \
     && (ICU_CCU6_SIGEDGE_USED == STD_ON)) */
...
/*******************************************************************************
** Syntax    :  static void Icu_lTimeStampHandler                             **
**                 (                                                          **
**                     uint8 ChannelType, uint8 Channel                       **
**                  )                                                         **
**                                                                            **
...
**                                                                            **
** Parameters (in):                                                           **
**             ChannelType -  Type of the current channel                     **
**             Channel -      Currnt Channel ID                               **
**                                                                            **
** Parameters (out)  None                                                     **
**                                                                            **
** Return value:   void                                                       **
**                                                                            **
** Description :   Function to invoke the respective notification when        **
**                 TimeStap mode is selected                                  **
**                 This function has been introduced in order to resolve the  **
**                 RSM warning (#55: exceeding the limit for the              **
**                 depth of scope)                                            **
*******************************************************************************/
static void Icu_lTimeStampHandler
(
...
)
{
    ...
    pointer arithmetic.*/        
    LocalIcuMapPtr =  (Icu_kConfigPtr->MappingIndexPtr) + Channel; 
    ChannelIndex = LocalIcuMapPtr->ChannelIndexMap;  
    ...
    {        
        ...
        CellNum = \
          (uint8)(LocalIcuChanPtr->IcuProperties.AssignedHwUnitNumber);
        #if(ICU_SAFETY_ENABLE == STD_ON)
        if (ICU_GTM_ASIL_CHANNEL == ChannelType)
        {
            #if (ICU_MAX_TIMESTAMP_ASIL_CHANNELS != 0U)              
           
            *(Icu_17_GtmCcu6_AsilGtmVarlist.TimeStampParas[ChannelIndex].\
                             Address) = (uint32)Icu_lGtmGetTimeStamp(CellNum);
            /*IFX_MISRA_RULE_17_04_STATUS=SFR and post build 
           access forces to use pointer arithmetic.*/            
            Icu_17_GtmCcu6_AsilGtmVarlist.TimeStampParas[ChannelIndex].\
                                                               Address++;
            Icu_17_GtmCcu6_AsilGtmVarlist.TimeStampParas[ChannelIndex].\
                                                               Counter++;
            ...
            */     
            ...
            {
                Notify = Icu_17_GtmCcu6_AsilGtmVarlist.\
                                       TimeStampParas[ChannelIndex].Notify;
                /* The notification should occur when counter 
                   reaches the next interval value */
                Icu_17_GtmCcu6_AsilGtmVarlist.\
                          TimeStampParas[ChannelIndex].NotifyCounter += Notify;
                if ( ICU_ENABLE_NOTIFICATION == \
                Mcal_Extract(
        ...
        #endif
        {
            ...
            *(Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].Address) = \
                             (uint32)Icu_lGtmGetTimeStamp(CellNum);
            /*IFX_MISRA_RULE_17_04_STATUS=SFR and post build access forces to 
              use pointer arithmetic.*/            
            Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].Address++;
            Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].Counter++;
            ...
            Counter = \
                 Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].Counter;
            NotifyCounter = \
                Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].\
                                                                  NotifyCounter;
            if (Counter == NotifyCounter) 
            {
                Notify = \
                   Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].Notify;
                /* The notification should occur when counter 
                   reaches the next interval value */
                Icu_17_GtmCcu6_QmVarlist.TimeStampParas[ChannelIndex].\
                                                       NotifyCounter += Notify;
                if (ICU_ENABLE_NOTIFICATION == \
                     Mcal_Extract(
        ...
        /* Check the Timestamp Buffer limit */
        Icu_lTimeStampBufferLimitCheck(
        MeasureKind, CellNum, ChannelIndex, ChannelType);     
    }    
}
#endif
...
/* Compiler switch modified to fix the Jira 0000051018-566 */
#if ((ICU_TIM_TIMSTMP_USED == STD_ON) || \
     ((ICU_TIM_SIGMEAS_USED == STD_ON) && \
     ((ICU_GET_INPUT_STATE_API == STD_ON)|| \
     (ICU_GET_TIME_ELAPSED_API == STD_ON))))
  ...
  if(Channel < MaxChannels)
  {
    ...
    switch (LocalIcuChanPtr->IcuProperties.MeasurementMode) 
    {
      ...
      #if (ICU_TIM_TIMSTMP_USED == STD_ON)
      {
        Icu_lTimeStampHandler(ChlType, Channel);
        /* This function has been introduced in order to 
        resolve the RSM warning (#55: exceeding the limit 
        for the depth of scope)*/
      } 
                                      ...
                                      Channel, uint32 EdgeCount)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS=Pointer arithmetic used due to 
    ...
    /* check for the valid range for EdgeCount parameter. 
    Upper limit check is introduced as part of Jira fix 0000051018-496*/
    else if ((EdgeCount == 0U) || (EdgeCount > ICU_EDGE_COUNT_MAX_VALUE))
    {
      ErrorFlag = ICU_ERROR_FOUND;
     #if (ICU_DEV_ERROR_DETECT == STD_ON)  
      Det_ReportError( 
