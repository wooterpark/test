...
{
    // TRACE[BSW_SWS_AR4_0_R2_NVRAMManager_Ext-2943] Service used from an auxiliary user to copy the data of the NV block to its corresponding RAM block
    ...
    else if (!NvM_Prv_IsBlockIdValid(NVM_SERVICE_ID_RB_AUX_READ_BLOCK,
                                     BlockId,
    ...
    else
    {
        ...
        if ((NvM_Prv_stRequests_rAMwAM_au16[NVM_PRV_AUX_ADMIN_BLOCK] == 0) && (NvM_Prv_Main_st.idActiveBlockAdmin_rAMwM_u16 != NVM_PRV_AUX_ADMIN_BLOCK))
        {
            ...
            if (NvM_DstPtr != NULL_PTR)
            {
                ...
                uint8 idService_u8 = (uint8) NVM_SERVICE_ID_RB_AUX_READ_BLOCK;
                    ...
                    (NvM_Prv_idxDataSet_rAMwA_au8[NVM_PRV_AUX_ADMIN_BLOCK] >= NvM_Prv_BlockDescriptors_acst[BlockId].nrNvBlocks_u8))
                {
                    ...
                    idService_u8 = NVM_SERVICE_ID_RB_AUX_RESTORE_BLOCK_DEFAULTS;
                ...
                NvM_Prv_QueueAuxBlockRequest(idService_u8, BlockId, (uint8 *) NvM_DstPtr);
                ...
                NvM_Prv_stRequests_rAMwAM_au16[NVM_PRV_AUX_ADMIN_BLOCK] |= stRequest_u16;
            ...
            {
#if (NVM_DEV_ERROR_DETECT == STD_ON)
                // TRACE[NVM616] Report the DET error NVM_E_PARAM_ADDRESS when a NULL pointer is passed via the parameter NvM_DstPtr
                flgDetError_b = TRUE;
    ...
    if (flgDetError_b != FALSE)
    {
        ...
        NVM_PRV_REPORT_DET_ERROR(NVM_SERVICE_ID_RB_AUX_READ_BLOCK, idDetError_u8);
    }
#endif
