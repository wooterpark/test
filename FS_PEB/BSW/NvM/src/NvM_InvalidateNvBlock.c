...
#endif
{
    ...
    else if (!NvM_Prv_IsBlockIdValid(NVM_SERVICE_ID_INVALIDATE_NV_BLOCK, BlockId, BlockId))
    {
    ...
    else
    {
             ...
             (NvM_Prv_idxDataSet_rAMwA_au8[BlockId] < NvM_Prv_BlockDescriptors_acst[BlockId].nrNvBlocks_u8)))
        {
            ...
            // TRACE[BSW_SWS_AR4_0_R2_NVRAMManager_Ext-2866] Exception to NVM385: NvM_InvalidateNvBlock can be queued for a block whose the only pending operation is a a maintenance operation.
               ...
               )
            {
                ...
                if (NvM_Prv_QueueBlockRequest(NVM_SERVICE_ID_INVALIDATE_NV_BLOCK, BlockId, NULL_PTR) == NVM_PRV_QUEUE_OP_OK)
                {
                    ...
                    NvM_Prv_stRequests_rAMwAM_au16[BlockId] |= (uint16) NVM_PRV_BLOCK_REQ_INVALIDATE;
        ...
        {
            // TRACE[BSW_SWS_AR4_0_R2_NVRAMManager_Ext-2818] Report an error by setting a block error flag if an invalidation is attempted for a write protected block
            // To be compliant with NVM377, do the same if an invalidation of a ROM block of an NvM-block of type DATASET is attempted
    ...
    if (flgDetError_b != FALSE)
    {
        ...
        NVM_PRV_REPORT_DET_ERROR(NVM_SERVICE_ID_INVALIDATE_NV_BLOCK, idDetError_u8);
    }
#endif
