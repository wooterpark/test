...
LOCAL_INLINE boolean  E2E_Prv_P06CheckConfigType(const E2E_P06ConfigType * const Config_pst);
LOCAL_INLINE boolean  E2E_Prv_P06CheckData(const E2E_P06ConfigType * const Config_pst, const uint8 * const Data_pu8,
                                           uint16 Length_u16);
LOCAL_INLINE uint8    E2E_Prv_P06ReadU8(const uint8 * const Data_pu8, uint16 offset_u16);
LOCAL_INLINE void     E2E_Prv_P06WriteU8(uint8 * const Data_pu8, uint16 offset_u16, uint8 value_u8);
LOCAL_INLINE uint16   E2E_Prv_P06ReadU16(const uint8 * const Data_pu8, uint16 offset_u16);
LOCAL_INLINE void     E2E_Prv_P06WriteU16(uint8 * const Data_pu8, uint16 offset_u16, uint16 value_u16);
LOCAL_INLINE uint16   E2E_Prv_P06CalcCRC16(const E2E_P06ConfigType * const Config_pst, const uint8 * const Data_pu8,
                                           uint16 Length_u16);
...
/**
 ***********************************************************************************************************************
 * E2E_Prv_P06CheckConfigType
 *
 * \brief Tests the input parameter E2E_P06ConfigType for correctness
 *
 * Description: Tests the input parameter E2E_P06ConfigType for correctness
 ...
 * The E2_P06Protect or E2E_P06Check function is interrupted and returns an error message.
 *
 * Restrictions:
 *   -
 *
...
LOCAL_INLINE boolean E2E_Prv_P06CheckConfigType(const E2E_P06ConfigType * const Config_pst)
{
    return ((boolean)(((Config_pst->Offset % 8U) == 0U) &&
                      ...
                      (Config_pst->MinDataLength >= E2E_P06_HEADER_SIZE) &&
                      ((Config_pst->MaxDataLength % 8U) == 0U) &&
                      (Config_pst->MinDataLength <= Config_pst->MaxDataLength) &&
                      (Config_pst->Offset <= (Config_pst->MaxDataLength - E2E_P06_HEADER_SIZE))));
}
...
LOCAL_INLINE uint8 E2E_Prv_P06ReadU8(const uint8 * const Data_pu8, uint16 offset_u16)
{
    /* return U8 value with reversed bit order (bit order is flipped) */
    return (E2E_Prv_ReverseBitsU8(Data_pu8[offset_u16]));
}
...
LOCAL_INLINE void E2E_Prv_P06WriteU8(uint8 * const Data_pu8, uint16 offset_u16, uint8 value_u8)
{
    /* Write U8 value into the specified region of data (big endian order with bit order is flipped */
    Data_pu8[offset_u16] = E2E_Prv_ReverseBitsU8(value_u8);
}
                                         ...
                                         uint16 Length_u16)
{
    ...
    /* write Data ID into temporary buffer (big endian order and bit order is flipped) */
    dataID_au8[0U] = E2E_Prv_ReverseBitsU8((uint8)(Config_pst->DataID >> 8U));
    dataID_au8[1U] = E2E_Prv_ReverseBitsU8((uint8)(Config_pst->DataID));
