...
echo "1">>log.txt
::拷贝生成的代码至SWC中，注意查看路径
REM copy !codepath!\output\SWC_ADM_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_ADM
REM copy !codepath!\output\SWC_FRP_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_FRP
REM copy !codepath!\output\SWC_STM_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_STM
REM copy !codepath!\output\SWC_SWD_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_SWD
REM copy !codepath!\output\SWC_TCF_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_TCF
REM copy !codepath!\output\SWC_TRSP_ert_rtw\*.c !cur_path!\SWC\A_SWC\SWC_TRSP

REM copy !codepath!\output\SWC_ADM_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_ADM
REM copy !codepath!\output\SWC_FRP_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_FRP
REM copy !codepath!\output\SWC_STM_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_STM
REM copy !codepath!\output\SWC_SWD_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_SWD
REM copy !codepath!\output\SWC_TCF_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_TCF
REM copy !codepath!\output\SWC_TRSP_ert_rtw\*.h !cur_path!\SWC\A_SWC\SWC_TRSP

REM copy !codepath!\output\SWC_ADM_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_ADM
REM copy !codepath!\output\SWC_FRP_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_FRP
REM copy !codepath!\output\SWC_STM_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_STM
REM copy !codepath!\output\SWC_SWD_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_SWD
REM copy !codepath!\output\SWC_TCF_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_TCF
REM copy !codepath!\output\SWC_TRSP_ert_rtw\*.a2l !cur_path!\SWC\A_SWC\SWC_TRSP
::####################################################################################################
echo "2">>log.txt
::获取mk文件里的最长的那个长链接
for /r %%i in (*.h) do (
...
echo 获取mk文件里的最长的那个长链接,successful! >>log.txt
del list_c_h.txt
...
echo 长链接分割成短链接,successful! >>log.txt
::####################################################################################################
echo "4">>log.txt
::复制所有.c文件至rom下
...
del list_c.txt
echo 复制所有.c文件至rom下,successful! >>log.txt
::####################################################################################################
::删除excluding不用的
echo "5">>log.txt
...
echo 删除excluding不用的字符串,successful! >>log.txt
::####################################################################################################
echo "6">>log.txt
...
del ROM_c_route.txt
echo 删除后，获取所有有用.c文件和路径,并去重复,successful! >>log.txt
::####################################################################################################
echo "7">>log.txt
::在去重复的路径下生成subdir.mk文件
...
echo 在去重复的路径下生成mk文件,successful! >>log.txt
::####################################################################################################
::生成makefile文件
echo "8">>log.txt
...
echo 生成makefile文件开头,successful! >>log.txt
echo #>>%var12%makefile
...
echo		"$(TRICORE_TOOLS)/bin/tricore-gcc" -o  "PEB.elf" -nocrt0 -nostdlib -nostartfiles -T"../TOOLS/Linker/LINKER.ld" @ROM.objectlist -mcpu=tc27xx -Wl,--warn-section-align -Wl,--warn-orphan -L"E:\FS_PEB\OS\Output" -L"E:\FS_PEB\TOOLS\Linker" -l:RTAOS.a -l:libgcov.a -l:libgcc.a -l:libdnk_c.a -Wl,-Map="$(basename $(notdir $@)).map" -Wl,--relax -Wl,--no-demangle -Wl,--warn-once -nodefaultlibs -Wl,--extmap="a">>%var12%makefile	
...
echo #>>%var12%makefile
echo 生成makefile文件完整,successful! >>log.txt
::####################################################################################################
::生成objectlist
echo "9">>log.txt
for /f "delims=" %%i in (ROM_c.txt) do (
...
del mk_filepath.txt
del ROM_c.txt
echo 生成objectlist,successful! >>log.txt
::####################################################################################################
::生成sources.mk
echo "10">>log.txt
...
echo TRICORE_TOOLS :=C:/HIGHTEC/toolchains/tricore/v4.6.6.1>>%var12%sources.mk
::TRICORE_TOOLS路径需修改
echo #>>%var12%sources.mk
...
for /f "delims=" %%i in (mk_filepath.txt) do (
	 set var14=%%i
	 set "var14=!var14:%cur_path%\ROM\=!"
	 set "var14=!var14:\=/!"
	 echo !var14!>>%var12%sources.mk
)
echo 生成sources.mk,successful! >>log.txt
::####################################################################################################
::objects.mk
echo "11">>log.txt
...
echo 生成objects.mk,successful! >>log.txt
::####################################################################################################
::运行make -j8 rebuild
echo "12">>log.txt
echo 开始运行make -j8 rebuild,successful! >>log.txt
cmd /k "cd /d %cur_path%\ROM\&&make -j8 rebuild"
echo 结束运行make -j8 rebuild,successful! >>log.txt
del log.txt